...
namespace lang {
  ...
  Source::Source(CString n, CString t): name(std::move(n)), text(std::move(t)) {
    line_starts.push_back(0);
    for (size_t i = 0; i < text.size(); ++i)
      if (text[i] == '\n') line_starts.push_back(i + 1);
  }

  Loc Source::loc(size_t index) const {
    Loc result;
  ...
  CString Source::line(int ln) const {
  ...
  void err(const Loc& loc, const CString& msg, int type) {
